 ...
 */
export default function VideoBlock({ src, youtube, vimeo, placeholder, label = "Watch it in action", thumbnail, height = 480 }) {
  ...
  return (
    ...
    >
      {/* Dark overlay */}
      {thumbnail && (
        <div style={{ position:"absolute", inset:0, background:"rgba(44,53,48,0.45)" }} />
      )}
      ...
      {!placeholder ? (
        <div style={{
          ...
          <div style={{ width:0, height:0, borderTop:"12px solid transparent", borderBottom:"12px solid transparent", borderLeft:`20px solid ${c.sageD}`, marginLeft:4 }} />
        </div>
      ) : (
      ...
      <div style={{
        ...
        {placeholder ? (
          <>
            <div style={{ fontSize:13, opacity:0.8 }}>📹 Video coming soon</div>
            <div style={{ fontSize:11, opacity:0.6, marginTop:4, fontWeight:300 }}>Add your video URL in VideoBlock component</div>
          </>
        ) : label}
      </div>
    </div>
