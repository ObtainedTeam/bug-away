...
export default function Cart({ isOpen, onClose }) {
  ...
  if (hasMenJacket && !hasMenPants) {
    upsell = { text:"Add the matching pants and save", product:{ id:'ba-pants-men', name:'Bug Away Pants — Men', price:SINGLE_PRICE, images:['/images/lifestyle-men-outdoor.png'], placeholderBg:'#C8E0EA', colors:['Sage Green'] } };
  } else if (hasMenPants && !hasMenJacket) {
    upsell = { text:"Add the matching jacket and save", product:{ id:'ba-jacket-men', name:'Bug Away Jacket — Men', price:SINGLE_PRICE, images:['/images/product-men-jacket-front.png'], placeholderBg:'#A8CBB5', colors:['Sage Green'] } };
  } else if (hasWomenJacket && !hasWomenPants) {
    upsell = { text:"Add the matching pants and save", product:{ id:'ba-pants-women', name:'Bug Away Pants — Women', price:SINGLE_PRICE, images:['/images/lifestyle-women-outdoor.png'], placeholderBg:'#B8D4CC', colors:['Sage Green'] } };
  } else if (hasWomenPants && !hasWomenJacket) {
    upsell = { text:"Add the matching jacket and save", product:{ id:'ba-jacket-women', name:'Bug Away Jacket — Women', price:SINGLE_PRICE, images:['/images/lifestyle-women-outdoor.png'], placeholderBg:'#E8F4F8', colors:['Sage Green'] } };
  }
      ...
      <div style={{ position:'fixed', top:0, right:isOpen?0:`-${panelWidth}`, width:panelWidth, height:'100%', background:'#fff', zIndex:301, transition:'right 0.3s ease', display:'flex', flexDirection:'column', boxShadow:'-4px 0 24px rgba(0,0,0,0.12)' }}>
        ...
        <div style={{ padding:'18px 20px', borderBottom:`1px solid ${c.glL}`, display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <span style={{ fontSize:14, fontWeight:700, color:c.dark, fontFamily:"'Archivo',sans-serif", letterSpacing:"-0.01em" }}>Your Cart</span>
            {count > 0 && (
              <span style={{ background:c.sageD, color:'#fff', borderRadius:20, padding:'1px 8px', fontSize:11, fontFamily:"'Poppins',sans-serif", fontWeight:600 }}>{count}</span>
            )}
          </div>
        ...
        <div style={{ padding:'14px 20px', background:c.off, borderBottom:`1px solid ${c.glL}`, flexShrink:0 }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', marginTop:5 }}>
            <span style={{ fontSize:10, fontFamily:"'Poppins',sans-serif", color:c.gray }}>{symbol === '$' ? '$' : '€ '}0</span>
            <span style={{ fontSize:10, fontFamily:"'Poppins',sans-serif", color:c.gray }}>Free gift at {symbol === '$' ? '$' : '€ '}{FREE_SHIPPING_THRESHOLD}</span>
          </div>
        </div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'16px 20px' }}>
          {items.length === 0 ? (
            <div style={{ textAlign:'center', padding:'48px 20px' }}>
              ...
              <Link to="/shop" onClick={onClose} style={{ background:c.sageD, color:'#fff', padding:'11px 24px', borderRadius:4, fontSize:12, fontFamily:"'Poppins',sans-serif", fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', textDecoration:'none', display:'inline-block' }}>
                Shop Now
              </Link>
            </div>
          ) : (
            items.map(item => (
              <div key={item.key} style={{ display:'flex', gap:12, marginBottom:16, paddingBottom:16, borderBottom:`1px solid ${c.glL}` }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:11, fontFamily:"'Poppins',sans-serif", color:c.gray, marginBottom:8 }}>{item.color} · Size {item.size}</div>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    <div style={{ display:'flex', alignItems:'center', border:`1px solid ${c.glL}`, borderRadius:4, overflow:'hidden' }}>
                      <button onClick={() => CartContext.updateQty(item.key, item.quantity - 1)} style={{ width:28, height:28, background:'none', border:'none', cursor:'pointer', fontSize:14, color:c.dark }}>−</button>
                    ...
                    <span style={{ fontSize:14, fontWeight:700, color:c.dark }}>{formatPrice(item.product.price * item.quantity, symbol)}</span>
                  </div>
                </div>
                <button onClick={() => CartContext.remove(item.key)} style={{ background:'none', border:'none', cursor:'pointer', color:c.gray, fontSize:16, alignSelf:'flex-start', flexShrink:0 }}>🗑</button>
              </div>
            ))
          )}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, fontFamily:"'Poppins',sans-serif", color:c.grayD }}>
                    <span style={{ textDecoration:'line-through', color:c.gray, marginRight:4 }}>{formatPrice(upsell.product.price, symbol)}</span>
        ...
        {items.length > 0 && (
          <div style={{ borderTop:`1px solid ${c.glL}`, flexShrink:0 }}>
            <div style={{ display:'flex', borderBottom:`1px solid ${c.glL}` }}>
              <button onClick={() => { setShowDiscount(!showDiscount); setShowNotes(false); }} style={{ flex:1, padding:'12px', background:'none', border:'none', borderRight:`1px solid ${c.glL}`, cursor:'pointer', fontSize:12, fontFamily:"'Poppins',sans-serif", color:showDiscount?c.sageD:c.grayD, fontWeight:showDiscount?600:400 }}>
                🏷 Discount
              </button>
              <button onClick={() => { setShowNotes(!showNotes); setShowDiscount(false); }} style={{ flex:1, padding:'12px', background:'none', border:'none', cursor:'pointer', fontSize:12, fontFamily:"'Poppins',sans-serif", color:showNotes?c.sageD:c.grayD, fontWeight:showNotes?600:400 }}>
                📝 Notes
              </button>
            </div>
            {showDiscount && (
              <div style={{ padding:'12px 20px', borderBottom:`1px solid ${c.glL}`, display:'flex', gap:8 }}>
                <input type="text" placeholder="Discount code" value={discountCode} onChange={e => setDiscountCode(e.target.value)} style={{ flex:1, padding:'8px 12px', border:`1px solid ${c.glL}`, borderRadius:4, fontSize:12, fontFamily:"'Poppins',sans-serif", outline:'none' }} />
                <button style={{ background:c.sageD, color:'#fff', border:'none', padding:'8px 16px', borderRadius:4, fontSize:12, fontFamily:"'Poppins',sans-serif", fontWeight:600, cursor:'pointer' }}>Apply</button>
              </div>
            )}
            {showNotes && (
              <div style={{ padding:'12px 20px', borderBottom:`1px solid ${c.glL}` }}>
                <textarea placeholder="Add a note for your order..." value={notes} onChange={e => setNotes(e.target.value)} rows={3} style={{ width:'100%', padding:'8px 12px', border:`1px solid ${c.glL}`, borderRadius:4, fontSize:12, fontFamily:"'Poppins',sans-serif", outline:'none', resize:'none' }} />
              </div>
            )}
            <div style={{ padding:'16px 20px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                <span style={{ fontSize:13, fontFamily:"'Poppins',sans-serif", color:c.grayD }}>Subtotal</span>
                <span style={{ fontSize:15, fontWeight:700, color:c.dark }}>{formatPrice(total, symbol)}</span>
              </div>
              <p style={{ fontSize:11, fontFamily:"'Poppins',sans-serif", color:c.gray, marginBottom:14 }}>Taxes and shipping calculated at checkout</p>
              <button onClick={handleCheckout} style={{ width:'100%', background:c.sageD, color:'#fff', border:'none', padding:'14px', borderRadius:4, fontSize:13, fontFamily:"'Poppins',sans-serif", fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', cursor:'pointer', marginBottom:8 }}>
                Checkout →
              </button>
              <button onClick={onClose} style={{ width:'100%', background:'none', border:`1px solid ${c.glL}`, color:c.grayD, padding:'11px', borderRadius:4, fontSize:12, fontFamily:"'Poppins',sans-serif", cursor:'pointer' }}>
                Continue Shopping
              </button>
            </div>
          </div>
